...
//********************************************************
//  TimestampTags  used to store a timestamp with a packet
//  when they are placed in the queue
//********************************************************
TypeId TimestampTags ::GetTypeId (void)
{
  static TypeId tid = TypeId ("TimestampTags ")
    .SetParent<Tag> ()
    .AddConstructor<TimestampTags > ()
    .AddAttribute ("Timestamp",
                   "Some momentous point in time!",
                   EmptyAttributeValue (),
                   MakeTimeAccessor (&TimestampTags ::GetTimestamp),
                   MakeTimeChecker ())
  ;
  return tid;
}

TypeId TimestampTags ::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t TimestampTags ::GetSerializedSize (void) const
{
  return 8;
}

void TimestampTags ::Serialize (TagBuffer i) const
{
  int64_t t = m_timestamp.GetNanoSeconds ();
  i.Write ((const uint8_t *)&t, 8);
}

void TimestampTags ::Deserialize (TagBuffer i)
{
  int64_t t;
  i.Read ((uint8_t *)&t, 8);
...
void TimestampTags ::SetTimestamp (Time time)
{
  m_timestamp = time;
}

Time TimestampTags ::GetTimestamp (void) const
{
  return m_timestamp;
}

void TimestampTags ::Print (std::ostream &os) const
{
  os << "t=" << m_timestamp;
}
...
SimpleWirelessNetDevice::TransmitComplete (void)
{
  ...
  NS_ASSERT_MSG(m_currentPkt != nullptr, "SimpleWirelessNetDevice::TransmitComplete(): m_currentPkt zero");
