...
def main():
    ...
    max_lambda_exp = -2
    step_size = 1
    mcs2_values = [2, 4, 8]  # MCS values for link 2
    lambdas = [10 ** exp for exp in range(min_lambda_exp, max_lambda_exp + 1, step_size)]

    # Initialize data structures to store results
    # delays_data: {mcs2: {'lambda': [], 'queuing_delay_link1': [], ... }}
    delays_data = {}
    ...
    for mcs2 in mcs2_values:
        ...
        for lambda_val in lambdas:
            cmd = f"./ns3 run 'single-bss-mld --rngRun={rng_run} --payloadSize={payload_size} --mldPerNodeLambda={lambda_val} --mcs2={mcs2}'"
            print(f"Running simulation with mcs2={mcs2}, lambda={lambda_val}")
            ...
            with open('wifi-mld.dat', 'r') as f:
                ...
                # Store the results
                delays_data[mcs2]['lambda'].append(lambda_val)
                delays_data[mcs2]['queuing_delay_link1'].append(queuing_delay_link1)
                delays_data[mcs2]['queuing_delay_link2'].append(queuing_delay_link2)
                delays_data[mcs2]['queuing_delay_total'].append(queuing_delay_total)
                delays_data[mcs2]['access_delay_link1'].append(access_delay_link1)
                delays_data[mcs2]['access_delay_link2'].append(access_delay_link2)
                delays_data[mcs2]['access_delay_total'].append(access_delay_total)
                delays_data[mcs2]['e2e_delay_link1'].append(e2e_delay_link1)
                delays_data[mcs2]['e2e_delay_link2'].append(e2e_delay_link2)
                delays_data[mcs2]['e2e_delay_total'].append(e2e_delay_total)

    # Now plot the delays vs lambda for each mcs2 value
    for delay_type in ['queuing_delay', 'access_delay', 'e2e_delay']:
        for link in ['link1', 'link2', 'total']:
            plt.figure(figsize=(10, 6))
            for mcs2 in mcs2_values:
                lambda_values = delays_data[mcs2]['lambda']
                delay_values = delays_data[mcs2][f'{delay_type}_{link}']
                ...
                plt.plot(lambda_values, delay_values, marker='o', label=f"MCS2={mcs2}")
